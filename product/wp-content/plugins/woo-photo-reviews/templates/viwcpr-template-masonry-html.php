...
foreach ( $my_comments as $v ) {
	...
	if ( $product ) {
		...
		if ( is_array( $img_post_ids ) && count( $img_post_ids ) > 0 ) {
            ...
            <div class="reviews-images-container">
					...
					if ( count( $img_post_ids ) > 1 ) {
						foreach ( $img_post_ids as $img_post_ids_k => $img_post_id ) {
							if ( ! wc_is_valid_url( $img_post_id ) ) {
								...
								echo sprintf('<div class="reviews-images-wrap"><a data-image_index="%s" data-image_src="%s" data-image_caption="" rel="nofollow" href="%s"><img class="reviews-images" src="%s" alt="%s"/></a></div>',
								...
								echo sprintf( '<div class="reviews-images-wrap"><a data-image_index="%s" href="%s"><img class="reviews-images" src="%s" alt="%s"></a></div>',
				...
				if (! wc_is_valid_url( $first_ele )){
					...
					echo sprintf('<div class="reviews-images-wrap-right"><img class="reviews-images" data-original_src="%s" src="%s" alt="%s"  width="%s" height="%s"></div>',
					...
					echo sprintf('<div class="reviews-images-wrap-right"><img class="reviews-images" src="%s" alt="%s"></div>',
						esc_url( $first_ele ),esc_attr( $product_title ) );
				}
				if ( count( $img_post_ids ) > 1 ) {
					echo sprintf('<div class="images-qty">+%s</div>',count( $img_post_ids ) - 1 );
				}
				?>
            </div>
			<?php
		...
		echo sprintf( '<div class="review-content-container">' );
		if ( '0' === $v->comment_approved ){
			echo sprintf('<p class="meta"><em class="woocommerce-review__awaiting-approval">%s</em></p>',__( 'Your review is awaiting approval', 'woo-photo-reviews' ));
		}else{
			?>
