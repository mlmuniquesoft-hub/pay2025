<?php
/*
Element: Video Player
*/
class vcVideoPlayer extends WPBakeryShortCode {

    function __construct() {
        $this->_mapping();
        add_shortcode( 'kapee_video_player', array( $this, '_html' ) );
	}
	public function _mapping() {
		if ( !defined( 'WPB_VC_VERSION' ) ) { return; }	
		vc_map( array(
			'name'			=> esc_html__( 'Video Player', 'kapee-extensions' ),
			'base'			=> 'kapee_video_player',
			'category' 		=> esc_html__( 'Kapee', 'kapee-extensions' ),
			'description' 	=> esc_html__( 'Popup video player.', 'kapee-extensions' ),
        	'icon' 			=> KAPEE_URI.'/inc/admin/assets/images/vc-icon.png',
			'params' 		=> array(
				//General
				array(
					'type' 			=> 'textfield',
					'heading' 		=> esc_html__( 'Video URL', 'kapee-extensions' ),
					'param_name' 	=> 'video_url',
					'description' 	=> esc_html__( 'Youtube or Vimeo video URL.', 'kapee-extensions' ),
					'std' 			=> 'https://www.youtube.com/watch?v=7e90gBu4pas',
				),
				array(
					'type' 			=> 'attach_image',
					'heading' 		=> esc_html__('Image placeholder', 'kapee-extensions'),
					'param_name' 	=> 'image_placeholder',
				),
				( function_exists( 'vc_map_add_css_animation' ) ) ? vc_map_add_css_animation( true ) : '',
				array(
					'type' 			=> 'textfield',
					'heading' 		=> esc_html__( 'Extra class name', 'kapee-extensions' ),
					'param_name' 	=> 'el_class',
					'description' 	=> esc_html__( 'If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.', 'kapee-extensions' )
				),
				//Style
				array(
					'type' 			=> 'css_editor',
					'heading' 		=> esc_html__( 'CSS box', 'kapee-extensions' ),
					'param_name' 	=> 'css',
					'group' 		=> esc_html__( 'Design Options', 'kapee-extensions' )
				)
			),
		) );
	}
	
	public function _html( $atts, $content ) {
		$args = shortcode_atts( array(
			'video_url' 			=> 'https://www.youtube.com/watch?v=7e90gBu4pas',
			'image_placeholder' 	=> '',
			'css_animation' 		=> 'none',	
			'el_class'				=> '',
			'css'					=> '',
		), $atts );		
		extract($args);
		$class				= array();
		$class[]			= 'kapee-element';
		$class[]			= 'kapee-video-player';
		$class[]			= $el_class;	
		$class[]			= kapee_get_css_animation($css_animation);
		$css_class 			= vc_shortcode_custom_css_class( $css, ' ' );
		$class[]			= $css_class;
		$args['class'] 		= implode(' ',array_filter($class));
		
		$image_placeholder_src = '';
		if($image_placeholder){
			$image_placeholder_src = wp_get_attachment_image_src( $image_placeholder, 'full' );
			$image_placeholder_src = $image_placeholder_src[0];
		}
		
		$args['image_placeholder_src'] 		= $image_placeholder_src;
		ob_start();
			kapee_get_pl_templates('shortcodes/video-player',$args );	
		return ob_get_clean();
	}	
}
new vcVideoPlayer();
?>