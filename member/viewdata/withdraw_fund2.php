<?php
	...
	if($countU==0){
		...
		if($dfgjkdf['type']=='Withdraw'){
			...
			$messager="Withdraw Verified - Pending Admin Payment";
		}else{
			$method=$uiyeri[15];
			$hfgsd=strlen($method);
		...
		// Execute the base64 encoded query to insert into trans_receive with 'Pending' status
		$decodedQuery = base64_decode($dfgjkdf['withdraw_que']);
		
		$insertResult = $mysqli->query($decodedQuery);
