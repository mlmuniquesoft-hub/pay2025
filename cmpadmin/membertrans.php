                            ...
                            <div class="row">
									<div class="panel panel-default">
										<div class="panel-heading">
											<i class="fa fa-bar-chart-o fa-fw"></i> Total Member Transfer Summary (<?php echo date("d M-Y", strtotime($date)); ?>)
														...
														<table class="table table-bordered table-hover table-striped">
															<thead>
																<tr>
																	...
																	<th>Last Transfer To</th>
																	<th>Current Balance</th>
																</tr>
															</thead>
<?php
	$DATE3=date('Y-m-d');
	$t=$mysqli->query("SELECT DISTINCT `user_trans` FROM `trans_receive` WHERE `type`='Transfer'  AND DATE(`time`) BETWEEN '".$date."' AND '".$DATE3."'");				   
	...
	$werI=$mysqli->query("SELECT DISTINCT `user_trans` FROM `trans_receive` WHERE `type`='Transfer'  AND DATE(`time`) BETWEEN '".$date."' AND '".$DATE3."' ORDER BY `date` DESC LIMIT $set_limit, $limit");
	...
	while($JKSHFKS=mysqli_fetch_assoc($werI)){
		$withrwaUser=$JKSHFKS['user_trans'];
...
?>
															<tbody>
																<tr>
																	...
																	<td><?php echo mysqli_num_rows($mysqli->query("SELECT `user_trans` FROM `trans_receive` WHERE `user_trans`='".$withrwaUser."' AND `type`='Transfer' AND `account`='Member' ")); ?></td>
																	<td>$<?php 
																		$ertre12=mysqli_fetch_assoc($mysqli->query("SELECT SUM(ammount) as toyu FROM `trans_receive` WHERE `user_trans`='".$withrwaUser."' AND `type`='Transfer' AND `account`='Member' ")); 
																		echo $ertre12['toyu'];
																	?></td>
																	<td><?php 
																		$ertre112=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `trans_receive` WHERE `user_trans`='".$withrwaUser."' AND `type`='Transfer' AND `account`='Member' ORDER BY `date` DESC LIMIT 1")); 
																		echo $ertre112['date'];
																	?></td>
