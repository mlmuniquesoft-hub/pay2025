		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			<form>
			  ...
			  <div class="form-group">
				<p id="gamrTitle" class="text-center text-warning"></p>
			  </div>
			  <div class="form-group">
				<p id="gamrDesc" class="text-center text-success"></p>
			  </div>
    ...
    <div class="app-container expanded">
		...
		<?php require_once'menu.php'?>
            ...
            <div class="container-fluid">
                <div class="side-body padding-top">
                    ...
                    <div class="row  no-margin-bottom">
                            ...
                            <div class="row">
								<div class="panel panel-default">
									<div class="panel-heading">
										<i class="fa fa-bar-chart-o fa-fw"></i> Submit New Multi Challange Games
										<div class="pull-right"> </div>
									</div>
									<!-- /.panel-heading -->
									<div class="panel-body">
										<div class="row">
											<form class="form-horizontal" id="formElem" name="formElem" action="gameC_action.php" method="post">
												...
												<div class="form-group">
													<label class="col-sm-3 control-label">Question Series</label>
													<div class="col-sm-8">	
														<input  name="series" type="radio" value="7" checked />7 Question &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
														<input  name="series" type="radio" value="3"/>3 Question &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													</div>
												</div>
												...
												<div class="form-group">
													...
													<div class="col-sm-8">
														<select name="type" id="game_type" class="form-control">
															...
															<?php
																$ttyy=$multichallenge->query("SELECT * FROM `gametype` WHERE `active`='1'");
																while($hhggff=mysqli_fetch_assoc($ttyy)){
															?>
															<option value="<?php echo $hhggff['serial']; ?>"><?php echo $hhggff['type_name'] ; ?></option>
															<?php } ?>
														</select>
													</div>
												</div>
												<div class="form-group">
													...
													<div class="col-sm-8">	
														<input class="form-control" name="qquu" placeholder="Game Question"  id="comment" />
													</div>
												</div>
												<div class="form-group pok-hide">
												...
												<div class="form-group pok-hide draw">
												...
												<div class="form-group">
													...
													<div class="col-sm-8">				
														...
														<input type="hidden" name="tds" value="<?php echo "multichallenge"; ?>" />
													</div>
												</div>
												<div class="form-group">
						...
						<div class="col-sm-12">
							<div class="panel panel-default">
								<div class="panel-heading">
									<i class="fa fa-bar-chart-o fa-fw"></i> Running Multi Challange Games
									<div class="pull-right"> </div>
								</div>
								<!-- /.panel-heading -->
								<div class="panel-body">
									<div class="row">
										<div class="col-xs-12 col-sm-12">
											<button class="btn btn-info Gamees" data-hide="Qussd" data-show="Qussdd7">7 Question Series</button>
											<button class="btn btn-info Gamees" data-hide="Qussd" data-show="Qussdd3">3 Question Series</button>
											
											<div class="table-responsive Qussd" id="Qussdd7">
												<table class="table table-bordered table-hover table-striped">
													<thead>
														<tr>
															<th scope="col">Serial</th>										
															<th scope="col">Game Type</th>
															<th scope="col">Game Teams</th>
															<th scope="col">Win Team</th>
															<th scope="col">Action</th>											
															<th scope="col">Question For User</th>
															<th scope="col">Action</th>
														</tr>
													</thead>
				<?php
					$t = $multichallenge->query("SELECT * FROM `game_question` WHERE `active`='1' AND `user`='Company' AND `gameType`='7' ");
					...
					$q = $multichallenge->query("SELECT * FROM `game_question` WHERE `active`='1' AND `user`='Company' AND `gameType`='7' ORDER BY serial DESC LIMIT $set_limit, $limit");
													...
													<tbody class="del<?php echo $querygetexeres->serial; ?>">
														<tr>
															...
															<td style="min-width: 400px;"><?php 
																$crite=explode("/", $querygetexeres->teams);
																$criteIn=explode("/", $querygetexeres->criteria_inactive);
																$criteAmn=explode("/", $querygetexeres->special_reward);
																$coum=count($crite)-1;
																for($i=0;$i<=$coum;$i++){
																	$check='';
																	$check2='';
															...
															<td><?php 
																...
																for($i=0;$i<=$coum;$i++){
																	echo" 
																	<button class='btn btn-info' data-toggle='modal' data-target='#exampleModal' data-for='Win' data-serial='".$querygetexeres->serial."' data-whatever='".$crite[$i]."' data-ques='".$querygetexeres->question."' data-details='".$querygetexeres->detailss."' > Win ".$crite[$i]."</button>
																	<br/>";
																}
															?></td>											
															<td>
															...
															<td style="min-width: 200px;">
																<textarea class="form-control uupp" rows="6" data-serial="<?php echo $querygetexeres->serial; ?>" data-cols="question" ><?php echo $querygetexeres->question;?> </textarea>
															</td>
															<td style="min-width: 200px;">
																<textarea class="form-control uupp" rows="6" data-serial="<?php echo $querygetexeres->serial; ?>" data-cols="detailss" ><?php echo $querygetexeres->detailss;?> </textarea>
															</td>
															
															<td>
																<button href="#" class="btn btn-danger btn-sm upgg" data-toggle='modal' data-target='#exampleModal' data-serial='<?php echo $querygetexeres->serial; ?>' data-whatever='<?php echo $querygetexeres->short_ques;?>' data-for="Cancel">Cancel</button>
															</td>
														</tr>
													</tbody>
				<?php } ?>
				...
				?>
											...
											<div class="table-responsive Qussd" id="Qussdd3" style="display:none;">
												<table class="table table-bordered table-hover table-striped">
													<thead>
														<tr>
															<th scope="col">Serial</th>										
				...
				<?php
					$t = $multichallenge->query("SELECT * FROM `game_question` WHERE `active`='1' AND `user`='Company' AND `gameType`='3' ");
					$total_items= mysqli_num_rows($t);
					$limit=$_GET['limit'];
					$type=$_GET['type'];
					$page=$_GET['page'];
					...
					$total_pages= ceil($total_items / $limit);
					$set_limit=$page * $limit - ($limit);			
					$q = $multichallenge->query("SELECT * FROM `game_question` WHERE `active`='1' AND `user`='Company' AND `gameType`='3' ORDER BY serial DESC LIMIT $set_limit, $limit");
        ...
        <div>
			...
			<script type="text/javascript" src="js/jquery_ui.js"></script>
				...
				<script>
					$(".Gamees").on("click", function(){
						var hhgg=$(this).attr("data-hide");
						var ssgg=$(this).attr("data-show");
						$("."+hhgg).slideUp("slow");
						$("#"+ssgg).slideDown("slow");
					});
					$('#exampleModal').on('show.bs.modal', function (event) {
						...
						modal.find('#gamrTitle').text(ques);
						modal.find('#gamrDesc').text(details);
					...
					$("#game_submit").on("click", function(){
						...
						if(pin!=''){
							if(pin==pin2){
								if(tfg=="Win"){
									var trru="game_winc_done.php";
								}else if(tfg=="Cancel"){
									trru="gamec_cancel.php";
								}
								var reqqq=$.ajax({
									method:"GET",
									url: trru,
				  ...
				  $(document).ready(function(){
					  ...
					  $(".third").on("click", function(){
						  $("draw22").hide();
						  $(".draw").after( "<div class=\"form-group pok-hide draw22\" ><label class=\"col-sm-3 control-label\">Draw</label><div class=\"col-sm-5\"><input id=\"l_country\" name=\"criteria_active[]\"  class=\"form-control remov\"  type=\"text\" placeholder=\"Draw\" /></div></div>" );
					  })
					...
					$(document).ready(function(){
						$(".uupp").on("keyup", function(){
							...
							if(vals!=''){
								var reqq=$.ajax({
									method:"GET",
									url: "gameinfo_update_action.php",
									data:{vas: vals, sers: srl, coll:cols, tbs:"game_question"}
								});
								reqq.done(function(dffg){
									console.log(dffg);
								});
							}
						});
						...
						$(".upgg").on("click", function(e){
							...
							if(vals!=''){
								var reqq=$.ajax({
									method:"GET",
									url: "gameinfo_update_action.php",
									data:{vas: vals, sers: srl, coll:cols, tbs:"game_question",dts:"multichallenge"}
								});
							}
						});
