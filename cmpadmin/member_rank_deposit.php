                            ...
                            <div class="row">
									<div class="panel panel-default">
										<div class="panel-heading">
											<i class="fa fa-bar-chart-o fa-fw"></i> Total Member Rank Count (No Deposit) Summary
														...
														<table class="table table-bordered table-hover table-striped">
															<thead>
																<tr>
																	...
																	<th>Package:</th>
																	<th>Sponsored By:</th>
																	<th>Total Live Play:</th>         
																	<th>Total Board Play:</th>         
...
<?php
	...
	$t=$mysqli->query("SELECT `serial` FROM `noDeposit_user`");
	...
	$q=$mysqli->query("SELECT * FROM `noDeposit_user` ORDER BY serial DESC LIMIT $set_limit, $limit");
	$i=1;
	while($werw=mysqli_fetch_assoc($q)){
	   $mghh=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `member` WHERE `user`='".$werw['user']."'"));
	   $mghh321=mysqli_fetch_assoc($mysqli->query("SELECT `final_taka` FROM `balance` WHERE `user`='".$werw['user']."'"));
	   $mghh12=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `profile` WHERE `user`='".$mghh['log_user']."'"));
?>
															<tbody>
																<tr>
																	<td><?php echo $i++;; ?></td>     
																	<td><a class="btn btn-success" href="member_login.php?user=<?php echo $werw['user']; ?>" id="color"><?php echo $werw['user']; ?></a></td> 
																	<td><?php
																		if($mghh['pack']>0){
																			$uuu=$mghh['pack'];
																			settype($uuu, "integer");
