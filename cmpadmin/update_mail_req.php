<?php
	...
	if(!isset($_SESSION['Admin'])){
		...
		if(isset($_GET['piin'])){
			...
			if($mmCheck>0){
				if(count($ret)==0){
					$mnbb=mysqli_fetch_assoc($mysqli->query("SELECT `log_user` FROM `member` WHERE `user`='".$user."'"));
					$mnbb2=mysqli_fetch_assoc($mysqli->query("SELECT `email` FROM `member` WHERE `user`='".$mnbb['log_user']."'"));
					$hfjjk=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `email_reqq` WHERE `user`='".$user."' AND `active`='0'"));
					$refg=$hfjjk['req_for'];
					if($refg=="Mobile"){
						$colss="mobile";
					}else{
					...
					if($hfjjk['email']==''){
						array_push($ret, 0);
						array_push($ret, "$refg Not Valid");
						echo json_encode($ret);
						die();
					}
					if($mnbb2['email']==''){
						$mysqli->query("UPDATE `profile` SET `$colss`='".$hfjjk['email']."' WHERE `user`='".$mnbb['log_user']."' ");
						$mysqli->query("UPDATE `email_reqq` SET `active`='1' WHERE `user`='".$user."' ");
						$eemmnn=$hfjjk['email'];
						if($refg!="Mobile"){
							$to = $eemmnn;
							include("mmnbbh.php");
						...
						array_push($ret, 1);
						array_push($ret, "Update Successfully");
						echo json_encode($ret);
						die();
					}else{
						if($mnbb2['email']!=$hfjjk['email']){
							$mysqli->query("UPDATE `profile` SET `email`='".$hfjjk['email']."' WHERE `user`='".$mnbb['log_user']."' ");
							$mysqli->query("UPDATE `email_reqq` SET `active`='1' WHERE `user`='".$user."' ");
							$eemmnn=$hfjjk['email'];
							if($refg!="Mobile"){
								$to = $eemmnn;
							...
							array_push($ret, 0);
							array_push($ret, "Same As Previous $refg");
							echo json_encode($ret);
							die();
						}
					}
...
<?php
	$t =  $mysqli->query("SELECT * FROM email_reqq WHERE `active`='0' ");
	...
	$q =  $mysqli->query("SELECT * FROM email_reqq WHERE `active`='0' ORDER BY serial ASC LIMIT $set_limit, $limit");
	$n=1;
	...
	while($querygetexeres= mysqli_fetch_object($q))	
   {
	   ...
	   $mghh2=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `profile` WHERE `user`='".$mghh['log_user']."'"));
	   //var_dump($mghh2);
?>
															<tbody>
																<tr>
																	...
																	<td><?php echo $mghh['log_user']; ?></td>     
																	<td><?php echo $mghh2['name']; ?></td>     
																	<td><?php echo $mghh['sponsor']; ?></td>
																	<td><?php echo $mghh2['mobile']; ?></td>     
																	<td><?php echo $mghh2['email']; ?></td>     
																	<td><?php echo $querygetexeres->email; ?></td>     
																	<td>
																	<?php 
																		if($querygetexeres->active==0){
																	?>
																	<button type="button" class="btn btn-info btn-block" data-toggle="modal" data-target="#exampleModal" data-whatever="<?php echo $querygetexeres->user; ?>" data-tiil="<?php echo $querygetexeres->req_for; ?>">Approve <?php echo $querygetexeres->req_for; ?></button>
																		<?php }else{?>
																		<button type="button" class="btn btn-default btn-block" >Approved</button>
																		<?php } ?>
																	</td>
																</tr>
<?php } ?>
        ...
        <div>
			...
			<script>
				...
				$('#exampleModal').on('show.bs.modal', function (event) {
				  ...
				  var modal = $(this)
				  modal.find('.modal-title').text("Approve New "+tiil +' For '+ recipient)
