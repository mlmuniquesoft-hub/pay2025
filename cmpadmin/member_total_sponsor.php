<?php
	...
	if(!isset($_SESSION['Admin'])){
		...
		$date=date("Y-m-d");
		if(isset($_GET['uud'])){
			$sponsor=base64_decode($_GET['uud']);
		}else{
                            ...
                            <div class="row">
									<div class="panel panel-default">
										<div class="panel-heading">
											<i class="fa fa-bar-chart-o fa-fw"></i> Total Self Sponsor Of <?php echo $sponsor; ?>
										...
										<div class="panel-body">
											<div class="row">
												...
												<div class="col-sm-4">
													<?php
														$total_member=mysqli_num_rows($mysqli->query("SELECT `user` FROM `member` WHERE `sponsor`='".$sponsor."' "));
													?>
													<button type="button" class="btn btn-info">Total Sponsor (<?php echo $total_member; ?>)</button>
												</div>
											</div>
...
<?php
	...
	$t =  $mysqli->query("SELECT * FROM `member` WHERE `sponsor`='".$sponsor."' ");
	...
	$q =  $mysqli->query("SELECT * FROM `member` WHERE `sponsor`='".$sponsor."'  ORDER BY serial DESC LIMIT $set_limit, $limit");
															...
															<tbody>
																<tr>
																	...
																	<td><?php 
																		...
																		echo "<span style='color:#FFF;padding:10px;background-color:".$mmnnlk['color']."'>" . $mmnnlk['pack'] ."</span> <br/><br/>"; 
																	?></td>
																	<td><?php echo $querygetexeres->sponsor; ?></td> 
																	<td><?php echo $querygetexeres->upline; ?></td> 
																	<td><?php 
																	$mghh22=mysqli_fetch_assoc($mysqli->query("SELECT * FROM `upgrade` WHERE `user`='".$querygetexeres->user."' ORDER BY `serial` ASC"));
																	if($mghh22['user']!=''){echo date("d M-Y", strtotime($querygetexeres->time));}else{echo "Inactive";} ?></td>
																	<td><?php echo date("d M-Y", strtotime($querygetexeres->time)); ?></td>
																</tr>
<?php } ?>
															</tbody>
														</table>
