<?php
	...
	//$datetime1 = date_create($dfjkghdkfj['days']);
	$datetime1 = date_create("2020-01-24");
	...
	for($i=-1;$i<=$differDate;$i++){
		$DATE=date("Y-m-d", strtotime("2020-01-24"."+ $i days"));//$Alldate['date'];
		if($DATE!=$TIMNB){
			echo $DATE ."\n";
			...
			while($allmember=mysqli_fetch_assoc($mdfg)){
				...
				if(($yuryu['match']>0)||($yuryu['carry']>0)){
					...
					if($checkInc<1){
						...
						//echo "UPDATE `binary_income` SET `total_sale`='".$yuryu['users']."', `matching`='".$yuryu['match']."',`slot_match`='".$ToalAmnI."',`flash_match`='".$flashMn."',`carry_id`='".$yuryu['carryUser']."', `cary`='".$yuryu['carry']."' WHERE `user`='".$allmember['user']."' AND `date`='".$DATE."'";
						$mysqli->query("UPDATE `binary_income` SET `total_sale`='".$yuryu['users']."', `matching`='".$yuryu['match']."',`slot_match`='".$ToalAmnI."',`flash_match`='".$flashMn."',`carry_id`='".$yuryu['carryUser']."', `cary`='".$yuryu['carry']."' WHERE `user`='".$allmember['user']."' AND `date`='".$DATE."'");
					}
				}
			}
		}
