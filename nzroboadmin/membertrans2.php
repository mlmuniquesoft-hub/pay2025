...
<?php
	$date=date('Y-m-d');
	$t=$mysqli->query("SELECT * FROM `trans_receive` WHERE `type`='Withdraw'  AND DATE(`time`)='".$date."'");				   
	...
	$werI=$mysqli->query("SELECT * FROM `trans_receive` WHERE `type`='Withdraw'  AND DATE(`time`)='".$date."' ORDER BY `time` DESC LIMIT $set_limit, $limit");
	$i=1;
