                                ...
                                <div class="card red summary-inline">
                                    <div class="card-body">
                                        <div class="content">
											...
											<?php 
												$rtert=mysqli_fetch_assoc($mysqli->query("SELECT SUM(amount) AS `Astotal` FROM `game_configure` WHERE `game_type`='1'"));
												echo $rtert['Astotal'];
											?>
											</div>
                                            <div class="sub-title">Total Play Safe Mode</div>
                                        </div>
                                ...
                                <div class="card yellow summary-inline">
                                    <div class="card-body">
                                        <div class="content">
                                            <div class="title" style="font-size: 30px;">$ 
											<?php 
												$rtert=mysqli_fetch_assoc($mysqli->query("SELECT SUM(amount) AS `Pstotal` FROM `game_configure` WHERE `game_type`='2'"));
												echo $rtert['Pstotal'];
											?>
											</div>
                                            <div class="sub-title">Total Play Live-in Play</div>
                                        </div>
                                        <div class="clear-both"></div>
                                    </div>
                                </div>
                                    ...
                                    <div class="card-body">
                                        <div class="content">
                                            <div class="title" style="font-size: 30px;">$ <?php 
												$rtert=mysqli_fetch_assoc($mysqli->query("SELECT SUM(amount) AS `Gstotal` FROM `game_configure` WHERE `count_c`='2' "));
												echo $rtert['Gstotal'];
											?></div>
                                ...
                                <div class="card blue summary-inline">
                                    <div class="card-body">
                                        <div class="content">
                                            <div class="title" style="font-size: 30px;">$ 
											<?php 
												$rtert=mysqli_fetch_assoc($mysqli->query("SELECT SUM(amount) AS `Tstotal` FROM `game_configure` WHERE `count_c`='1'  "));
												echo $rtert['Tstotal'];
											?>
											</div>
                                ...
                                <div class="card blue summary-inline">
                                    <div class="card-body">
                                        <div class="content">
                                            <div class="title" style="font-size: 30px;">$ 
											<?php 
												$rtert=mysqli_fetch_assoc($mysqli->query("SELECT SUM(curent_bal) AS `xstotal` FROM `game_return` "));
												echo $rtert['xstotal'];
											?>
											</div>
                                            <div class="sub-title">Total Game Return</div>
                                        </div>
                                        <div class="clear-both"></div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
									<div class="panel panel-default">
										<div class="panel-heading">
											<i class="fa fa-bar-chart-o fa-fw"></i> Total Member Play Summary
...
<?php
	...
	$fdgfd22=maxMingAMEPLAYView('',$set_limit);
	$i=1;
...
?>
															<tbody>
																<tr>
																	...
																	<td>
																	<span class="text-success text-center">Total:<?php echo mysqli_num_rows($mysqli->query("SELECT `user` FROM `play` WHERE `user`='".$withrwaUser."' ")); ?></span><br/>
																	<span class="text-warning text-center">Safe Play:<?php echo mysqli_num_rows($mysqli->query("SELECT `user` FROM `play` WHERE `user`='".$withrwaUser."' AND `type_id`='1'")); ?></span><br/>
																	<span class="text-danger text-center">Live Play:<?php echo mysqli_num_rows($mysqli->query("SELECT `user` FROM `play` WHERE `user`='".$withrwaUser."' AND `type_id`='2'")); ?></span><br/>
																	</td>
																	<td><?php echo mysqli_num_rows($mysqli->query("SELECT `user` FROM `renew_game` WHERE `user`='".$withrwaUser."' ")); ?></td>
																	<td>$<?php 
